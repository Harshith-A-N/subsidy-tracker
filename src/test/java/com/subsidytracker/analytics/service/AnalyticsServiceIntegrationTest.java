...
public class AnalyticsServiceIntegrationTest {
    ...
    public void setUp() {
        ...
        // fundUtilizationByScheme() derives a scheme's total budget by summing
        // its RegionalBudget rows (Scheme.totalBudget is unused — see the note
        // on that field). Madurai + Coimbatore only total 19,000,000, so this
        // third row makes scheme1's regional allocations actually add up to
        // the 20,000,000 the tests expect.
        RegionalBudget budget1Extra = new RegionalBudget();
        budget1Extra.setScheme(scheme1);
        budget1Extra.setRegionName("Trichy");
        budget1Extra.setAllocatedBudget(new BigDecimal("1000000.00"));
        budget1Extra.setUtilizedBudget(BigDecimal.ZERO);
        entityManager.persist(budget1Extra);

        // scheme2 had no RegionalBudget rows at all, so its computed total
        // budget was 0 instead of the intended 18,000,000.
        RegionalBudget budget2Scheme2 = new RegionalBudget();
        budget2Scheme2.setScheme(scheme2);
        budget2Scheme2.setRegionName("Chennai");
        budget2Scheme2.setAllocatedBudget(new BigDecimal("18000000.00"));
        budget2Scheme2.setUtilizedBudget(BigDecimal.ZERO);
        entityManager.persist(budget2Scheme2);
